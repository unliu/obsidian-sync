...
当然，一些好的习惯，比如诊断问题要先打日志、先分析清楚问题再动代码，包括类似「 iOS 判断真机运行流畅度不能看 Debug executable 模式，或者至少连着线调试」这种非显性知识……大部分情况还是需要我们自己通过提示帮助模型获得。

面对不够好的模型，有哪些技巧？其实就和团队管理面对 junior 成员的逻辑是一样的：给于更明确的指令，告诉它要做什么不要做什么，提醒它可以向自己提期望（比如真机测试时）等等，因为如果不说，它真的会不知道/想不到。
·

## 技术能力的价值
单单的产品经理 + vibe coding，实际是做不到顺利完成复杂项目的，原因是：
1. AI 的全局判断能力仍然有限，表现出来就是「智商不稳定」，有时候会提出一些荒谬的方向（当然，其实人也也一样）、犯一些荒谬的错误
	1. 直接删掉重要的代码文件/数据（我也遇到过不止一次）
	2. 明明应该写在脚本里，却要求人类手动执行
	3. Ilya 描述的 bug1-bug2 死循环
	4. 有时候架构很精巧，但有时候犯一些关键性的架构设计错误
2. 这个时候就需要人类来补足它的「性格缺陷」，参与它的所有工作。包括产品方案、技术方案、修 bug 的思路……等等等等，而当人和它合作久了，就知道它容易犯什么错误，于是在前面的提示词就会自然磨合变化，实际达到无需值守/很少干预的效果。
3. 更本质的原因是，AI 不能做到跟人一样在脑海中存储所有真正有价值的上下文。它是一个技术天才和专才，但不是商业软件世界的全才。
